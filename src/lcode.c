...
/*
** Load a variable value without freeing address registers from indexed
** variables. Compound assignments still need those registers for storing.
*/
void luaK_loadvar (FuncState *fs, expdesc *var, expdesc *e) {
  *e = *var;
  switch (e->k) {
    case VLOCAL: {
      int temp = e->u.var.ridx;
...
void luaK_optionalindex (FuncState *fs, expdesc *t, expdesc *k) {
  int reg, niljump;
  luaK_exp2nextreg(fs, t);
  reg = fs->freereg - 1;
  niljump = luaK_jumpifnil(fs, t);
  luaK_indexed(fs, t, k);
  exp2reg(fs, t, reg);
  luaK_patchtohere(fs, niljump);
  t->k = VNONRELOC;
  t->u.info = reg;
  if (reg >= luaY_nvarstack(fs) && fs->freereg <= reg)
