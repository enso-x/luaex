...
static int llex (LexState *ls, SemInfo *seminfo) {
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '-': {  /* '-' or '--' (comment) */
        next(ls);
        if (check_next1(ls, '=')) return extendedtoken(ls, TK_SUBEQ);  /* '-=' */
      ...
      case '=': {
        ...
        else if (check_next1(ls, '>')) return extendedtoken(ls, TK_ARROW);  /* '=>' */
        else return '=';
      }
      case '<': {
        next(ls);
      ...
      case '/': {
        ...
        else if (check_next1(ls, '=')) return extendedtoken(ls, TK_DIVEQ);  /* '/=' */
        else return '/';
      }
      case '+': {
        next(ls);
        if (check_next1(ls, '=')) return extendedtoken(ls, TK_ADDEQ);  /* '+=' */
        else return '+';
      }
      case '*': {
        next(ls);
        if (check_next1(ls, '=')) return extendedtoken(ls, TK_MULEQ);  /* '*=' */
        else return '*';
      }
      case '?': {
        next(ls);
        if (check_next1(ls, '?')) return extendedtoken(ls, TK_COALESCE);  /* '??' */
        else if (check_next1(ls, '.')) return extendedtoken(ls, TK_QDOT);  /* '?.' */
        else return '?';
      }
      case '|': {
        next(ls);
        if (check_next1(ls, '>')) return extendedtoken(ls, TK_PIPE);  /* '|>' */
        else return '|';
      }
      case '~': {
        next(ls);
