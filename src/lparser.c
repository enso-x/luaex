...
static void calleeexp (LexState *ls, expdesc *v) {
  /* calleeexp -> primaryexp { '.' NAME | '[' exp ']' } */
  FuncState *fs = ls->fs;
  primaryexp(ls, v);
  for (;;) {
    switch (ls->t.token) {
...
static void pipelineargs (LexState *ls, expdesc *f, expdesc *first) {
  ...
  luaK_fixline(fs, line);
  fs->freereg = cast_byte(firstreg + 1);
}
...
static BinOpr compoundop (int token) {
  switch (token) {
...
static void compoundassign (LexState *ls, expdesc *var, BinOpr op) {
  FuncState *fs = ls->fs;
  expdesc oldv, rhs;
  int line = ls->linenumber;
  ...
  luaK_loadvar(fs, var, &oldv);
  luaK_infix(fs, op, &oldv);
  expr(ls, &rhs);
  luaK_posfix(fs, op, &oldv, &rhs, line);
  luaK_storevar(fs, var, &oldv);
}
